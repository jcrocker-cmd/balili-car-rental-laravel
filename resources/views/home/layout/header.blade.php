<header id="header">
  <nav>
    <div class="header-col-1 ">
      <a href="/" class="brand"><img src="images/LOGO.webp" alt="logo" class="logo"></a>


    </div>



    <div class="header-col-middle">
      <span>
        <div><a href="#" id="scroll-up-2">Home</a></div>
        <div><a href=".featured-section">Most Rented</a></div>
        <div><a href=".howitworks">How to Rent</a></div>
        <div><a href=".contact-us">Contact Us</a></div>
      </span>
      <div><a href="/home" class="px-3 py-3 text-white view-rental-btn">BOOK NOW</a></div>
    </div>


    <div class="home-sidebar-div">
      <a href="#" id="home-side-bar-btn" class="home-sidebar-toggle" aria-label="Open sidebar menu"><i
          class="fas fa-bars"></i></a>
    </div>


  </nav>
</header>


<div class="home-sidebar">
  <span class="close-btn" class="home-sidebar-toggle">&times;</span>
  <div>
    <a href="#" id="scroll-up-2">Home</a>
    <a href=".featured-section">Most Rented</a>
    <a href="#">About</a>
    <a href=".howitworks">How to Rent</a>
    <a href=".contact-us">Contact Us</a>
    <a href="#">Book Now</a>
  </div>
</div>